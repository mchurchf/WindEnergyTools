...
class planarAverageTools:
    ...
    def __init__(self,t_,zCell_,zFace_,dz_,nzCell_,nzFace_,bulkData_,profileData_):
        self.t = t_
        ...
        self.bulkData = bulkData_
        self.profileData = profileData_

        self.nAverageWindows = 0
        self.timeWindowStart = []
        self.timeWindowEnd = []
        self.timeWindowStartSecString = []
        self.timeWindowEndSecString = []
        self.timeWindowStartHrsString = []
        self.timeWindowEndHrsString = []
    ...
    def setUpTimeWindowMean(self,timeAverageStart,timeAverageEnd,timeAverageWindowWidth):
        ...
        iStart = np.argmin(np.abs(self.timeWindowStart - self.t[0]))
        iEnd = np.argmin(np.abs(self.timeWindowEnd - self.t[-1]))
        ...
        # Save the start and end time lists as strings to be used for making legends.
        for i in range(self.nAverageWindows):
            self.timeWindowStartSecString.append(str(self.timeWindowStart[i]) + ' s')
            self.timeWindowEndSecString.append(str(self.timeWindowEnd[i]) + ' s')
            self.timeWindowStartHrsString.append(str(self.timeWindowStart[i]/3600.0) + ' hrs')
            self.timeWindowEndHrsString.append(str(self.timeWindowEnd[i]/3600.0) + ' hrs')
    ...
    def packageSOWFA(self):

        # Reset the list of averaged variables.
        self.resetVariables()
        
        # Assign the data to named variables to make it easy to access.
        self.nAverageWindows = len(self.t)
        
       #self.q = self.bulkData[:,0]
       #self.theta_surf = self.bulkData[:,1]
       #self.uStar = self.bulkData[:,2]
       #self.wStar = self.bulkData[:,3]
       #self.L = self.bulkData[:,4]
       #self.zi = self.bulkData[:,5]
       #self.dpdx = self.bulkData[:,6]
       #self.dpdy = self.bulkData[:,7]
        
        self.u_cell = self.profileData[:,:,0]
        self.v_cell = self.profileData[:,:,1]
        self.w_cell = self.profileData[:,:,2]
        self.hvelmag_cell = np.sqrt(np.square(self.u_cell) + np.square(self.v_cell))
        self.windSpeed_cell = np.sqrt(np.square(self.u_cell) + np.square(self.v_cell))
        self.windDir_cell = np.arctan2(self.v_cell,self.u_cell)*(180.0/np.pi)
        self.theta_cell = self.profileData[:,:,3]
        self.mueff_cell = self.profileData[:,:,20]
        self.thetatheta_r_cell = np.zeros((np.shape(self.u_cell)))
        self.utheta_r_cell = self.profileData[:,:,4]
        self.vtheta_r_cell = self.profileData[:,:,5]
        self.wtheta_r_cell = self.profileData[:,:,6]
        self.uu_r_cell = self.profileData[:,:,7]
        self.vv_r_cell = self.profileData[:,:,10]
        self.ww_r_cell = self.profileData[:,:,12]
        self.uv_r_cell = self.profileData[:,:,8]
        self.uw_r_cell = self.profileData[:,:,9]
        self.vw_r_cell = self.profileData[:,:,11]
        self.k_r_cell = (0.5*(self.uu_r_cell + self.vv_r_cell + self.ww_r_cell))
        self.shearStressMag_r_cell = np.sqrt(np.square(self.vw_r_cell) + np.square(self.uw_r_cell))
        self.uuu_cell = np.zeros((np.shape(self.u_cell)))
        self.vvv_cell = np.zeros((np.shape(self.u_cell)))
        self.www_cell = self.profileData[:,:,18]
        self.utheta_sfs_cell = self.profileData[:,:,27]
        self.vtheta_sfs_cell = self.profileData[:,:,28]
        self.wtheta_sfs_cell = self.profileData[:,:,29]
        self.uv_sfs_cell = self.profileData[:,:,22]
        self.uw_sfs_cell = self.profileData[:,:,23]
        self.vw_sfs_cell = self.profileData[:,:,25]
        self.shearStressMag_sfs_cell = np.sqrt(np.square(self.vw_sfs_cell) + np.square(self.uw_sfs_cell))
    ...
    def packageAMRWind(self):
        ...
        self.v_cell = self.profileData[:,:,1]
        self.w_cell = self.profileData[:,:,2]
        self.hvelmag_cell = self.profileData[:,:,3]
        self.windSpeed_cell = np.sqrt(np.square(self.u_cell) + np.square(self.v_cell))
        self.windDir_cell = np.arctan2(self.v_cell,self.u_cell)*(180.0/np.pi)
        self.theta_cell = self.profileData[:,:,4]
        self.mueff_cell = self.profileData[:,:,5]
        self.thetatheta_r_cell = self.profileData[:,:,6]
        self.utheta_r_cell = self.profileData[:,:,7]
        self.vtheta_r_cell = self.profileData[:,:,8]
        self.wtheta_r_cell = self.profileData[:,:,9]
        self.uu_r_cell = self.profileData[:,:,10]
        self.vv_r_cell = self.profileData[:,:,11]
        self.ww_r_cell = self.profileData[:,:,12]
        self.uv_r_cell = self.profileData[:,:,13]
        self.uw_r_cell = self.profileData[:,:,14]
        self.vw_r_cell = self.profileData[:,:,15]
        self.k_r_cell = (0.5*(self.uu_r_cell + self.vv_r_cell + self.ww_r_cell))
        self.shearStressMag_r_cell = np.sqrt(np.square(self.vw_r_cell) + np.square(self.uw_r_cell))
        self.uuu_cell = self.profileData[:,:,16]
        self.vvv_cell = self.profileData[:,:,17]
        self.www_cell = self.profileData[:,:,18]
        self.utheta_sfs_cell = self.profileData[:,:,19]
        self.vtheta_sfs_cell = self.profileData[:,:,20]
        self.wtheta_sfs_cell = self.profileData[:,:,21]
        self.uv_sfs_cell = self.profileData[:,:,22]
        self.uw_sfs_cell = self.profileData[:,:,23]
        self.vw_sfs_cell = self.profileData[:,:,24]
        self.shearStressMag_sfs_cell = np.sqrt(np.square(self.vw_sfs_cell) + np.square(self.uw_sfs_cell))
    ...
    def timeWindowMeanSOWFA(self):
        ...
        for i in range(self.nAverageWindows):
           ...
           #self.q.append(np.mean(self.bulkData[jS:jE,0],axis=0))
           #self.theta_surf.append(np.mean(self.bulkData[jS:jE,1],axis=0))
           #self.uStar.append(np.mean(self.bulkData[jS:jE,2],axis=0))
           #self.wStar.append(np.mean(self.bulkData[jS:jE,3],axis=0))
           #self.L.append(np.mean(self.bulkData[jS:jE,4],axis=0))
           #self.zi.append(np.mean(self.bulkData[jS:jE,5],axis=0))
           #self.dpdx.append(np.mean(self.bulkData[jS:jE,6],axis=0))
           #self.dpdy.append(np.mean(self.bulkData[jS:jE,7],axis=0))
            ...
            self.hvelmag_cell.append(np.mean(np.sqrt(np.square(self.profileData[jS:jE,:,0]) + 
                                                     np.square(self.profileData[jS:jE,:,1])),axis=0))
            self.windSpeed_cell.append(np.mean(np.sqrt(np.square(self.profileData[jS:jE,:,0]) + 
                                                       np.square(self.profileData[jS:jE,:,1])),axis=0))
            self.windDir_cell.append(np.mean(np.arctan2(self.profileData[jS:jE,:,1],self.profileData[jS:jE,:,0])*(180.0/np.pi),axis=0))
            self.theta_cell.append(np.mean(self.profileData[jS:jE,:,3],axis=0))
            self.mueff_cell.append(np.mean(self.profileData[jS:jE,:,20],axis=0))
            self.thetatheta_r_cell.append(np.zeros((np.shape(self.u_cell))))
            self.utheta_r_cell.append(np.mean(self.profileData[jS:jE,:,4],axis=0))
            self.vtheta_r_cell.append(np.mean(self.profileData[jS:jE,:,5],axis=0))
            self.wtheta_r_cell.append(np.mean(self.profileData[jS:jE,:,6],axis=0))
            self.uu_r_cell.append(np.mean(self.profileData[jS:jE,:,7],axis=0))
            self.vv_r_cell.append(np.mean(self.profileData[jS:jE,:,10],axis=0))
            self.ww_r_cell.append(np.mean(self.profileData[jS:jE,:,12],axis=0))
            self.uv_r_cell.append(np.mean(self.profileData[jS:jE,:,8],axis=0))
            self.uw_r_cell.append(np.mean(self.profileData[jS:jE,:,9],axis=0))
            self.vw_r_cell.append(np.mean(self.profileData[jS:jE,:,11],axis=0))
            ...
            self.uuu_r_cell.append(np.zeros((np.shape(self.u_cell))))
            self.vvv_r_cell.append(np.zeros((np.shape(self.u_cell))))
            self.www_r_cell.append(np.mean(self.profileData[jS:jE,:,18],axis=0))
            self.utheta_sfs_cell.append(np.mean(self.profileData[jS:jE,:,27],axis=0))
            self.vtheta_sfs_cell.append(np.mean(self.profileData[jS:jE,:,28],axis=0))
            self.wtheta_sfs_cell.append(np.mean(self.profileData[jS:jE,:,29],axis=0))
            ...
            self.vw_sfs_cell.append(np.mean(self.profileData[jS:jE,:,25],axis=0))
